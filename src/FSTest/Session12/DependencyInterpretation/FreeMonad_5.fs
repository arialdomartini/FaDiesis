module FSharpBits.ParserCombinators.XPUG.FreeMonad_5
...
let getCart id f = GetCart (id, f)
let writeLine s f = WriteLine (s, f)
let value v = Value v
    ...
    getCart
        ...
        (fun cart ->
            if List.length cart.products > 10 then
                writeLine "Too many, I am sorry"  (fun () -> value 0m)
            else
