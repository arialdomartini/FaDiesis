...
let rec cataGift
    ...
    | Wrapped(gift, wrappingPaperStyle) ->
        let resultInner = recurse gift
        fWrapped (resultInner, wrappingPaperStyle)
...
let ``whats inside the sample values`` () =
    ...
    test <@ whatsInside bookGift = "a book" @>
...
let rec cataGiftTailRecur
    ...
    let recurse = cataGiftTailRecur fBook fChocolate fWrapped fBoxed fWithACard

    match gift with
    | Book book -> fBook book
    | Chocolate chocolate -> fChocolate chocolate
    | Wrapped(gift, wrappingPaperStyle) ->
        let newAcc = fWrapped  (acc, wrappingPaperStyle)
        recurse newAcc gift

    | Boxed gift ->
        let newAcc = fBoxed acc
        recurse newAcc gift

    | WithACard(gift, message) ->
        let newAcc = fWithACard (acc, message)
        recurse newAcc gift
